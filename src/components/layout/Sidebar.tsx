...
const NavItem: React.FC<NavItemProps> = ({
  ...
  return <NavLink to={to} className={({
    isActive
  }) => `flex items-center py-3 px-4 rounded-md transition-all ${isActive ? "bg-logaz-blue text-white" : "text-logaz-dark-gray hover:bg-logaz-blue/10"}`}>
...
const Sidebar: React.FC = () => {
  ...
  return <div className={`bg-white border-r border-logaz-gray h-screen flex flex-col ${collapsed ? "w-[76px]" : "w-64"} transition-all duration-300`}>
      ...
      <div className="p-3 border-t border-logaz-gray">
        <button onClick={() => setCollapsed(!collapsed)} className="flex items-center py-2 px-4 w-full text-logaz-dark-gray hover:bg-logaz-blue/10 rounded-md transition-all">
